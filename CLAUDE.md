...
- **Packet format**: raw `complex64` IQ bytes, exactly `CHUNK * 8` bytes per datagram. Both sides must agree on `CHUNK` (via `config.py`). At the default `FS=40_000_000` and `CHUNK=4096`, that's ~9760 packets/s and ~320 MB/s on loopback.
- **Baseband convention**: signals are generated in baseband Hz with **signed** carrier frequencies (`-FS/2 < fc < +FS/2`). The receiver UI applies a fixed cosmetic offset of `DISPLAY_OFFSET_HZ` (default +150 MHz) so the displayed band reads e.g. 130-170 MHz while internal math stays at baseband. Only axis ticks, labels, and freq-entry parsing apply the offset; DDC/decimation/recording paths use baseband Hz throughout.
